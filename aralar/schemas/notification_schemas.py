...
class SchedulingSchema(Schema):
    ...
    time_start = fields.String(
        validate=validate.Regexp(
            r"^([01]?[0-9]|2[0-3]):[0-5][0-9]$", error="Time must be in HH:MM format"
        ),
        allow_none=True,
    )
                ...
                if start_date_only == end_date_only:
                    ...
                    try:
                        ...
                        if end_time <= start_time:
                            abort(
                                400,
                                description="time_end must be after time_start when scheduling is on the same day",
                            )
                    except ValueError:
...
class DisplayStyleSchema(Schema):
    ...
    background_color = fields.String(
        validate=validate.Regexp(
            r"^#[0-9A-Fa-f]{6}$", error="Background color must be a valid hex color"
        ),
        load_default="#FFFFE0",
    )
    text_color = fields.String(
        validate=validate.Regexp(
            r"^#[0-9A-Fa-f]{6}$", error="Text color must be a valid hex color"
        ),
        load_default="#000000",
    )
...
class DisplaySchema(Schema):
    ...
    type = fields.String(
        required=True, validate=validate.OneOf(["banner", "card", "modal", "toast"])
    )
