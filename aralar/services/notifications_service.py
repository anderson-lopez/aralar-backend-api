...
class NotificationsService:
    ...
    def create_notification(self, data: Dict[str, Any], tzname: Optional[str] = None) -> str:
        ...
        default_has_content = bool(
            (locales_map.get(default_locale) or {}).get("data", {}).get("content")
        )
        ...
        # Add timestamps
        now = datetime.utcnow()
        notification_data = {**data, "created_at": now, "updated_at": now}
    ...
    def update_notification(
        self, notification_id: str, data: Dict[str, Any], tzname: Optional[str] = None
    ) -> bool:
            ...
            if start_date.date() == end_date.date():
                ...
                try:
                    ...
                    if end_time <= start_time:
                        abort(
                            400,
                            description="Time end must be after time start when scheduling is on the same day",
                        )
                except ValueError:
