...
from aralar.core.security import require_permissions
...
@blp.doc(
    summary="Get notification", description="Get notification by ID", security=[{"bearerAuth": []}]
)
...
@blp.doc(
    ...
    description="Toggle notification active status",
    security=[{"bearerAuth": []}],
)
...
@blp.response(200, NotificationListResponseSchema)
@blp.doc(
    summary="Get notifications by location",
    description="Get notifications by display location",
    security=[{"bearerAuth": []}],
)
